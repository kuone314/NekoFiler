...
impl FileListItem {
    pub fn new(base_info: &FileBaseInfo, is_selected: bool) -> Self {
        FileListItem {
            is_selected,
...
#[tauri::command]
pub fn set_focus_item(app_handle: AppHandle, paneidx: usize, focusitem: &str) -> () {
    let Ok(mut data_store) = PANE_DATA.lock() else {
        return;
    };

    data_store.pane_info_list[paneidx].init_focus_item = focusitem.to_string();
    let _ = app_handle.emit_all("update_path_list", &data_store.pane_info_list[paneidx]);
}
...
pub fn update_pane_data(app_handle: &tauri::AppHandle,pane_idx: usize, prev_init_focus_item: &String, pane_info: PaneInfo) {
    let Ok(mut data_store) = PANE_DATA.lock() else {
        return;
    };
...
pub fn update_file_list(app_handle: &tauri::AppHandle) {
    for pane_idx in 0..=1 {
        ...
        update_pane_data(app_handle,pane_idx,&prev_init_focus_item,pane_info.clone());
...
#[tauri::command]
pub fn sort_file_list(app_handle: AppHandle, paneidx: usize, sork_key: SortKey) {
    let Ok(mut data_store) = PANE_DATA.lock() else {
        return;
    };
...
#[tauri::command]
pub fn toggle_selection(app_handle: AppHandle, paneidx: usize, trg_idx: usize) -> () {
    let Ok(mut data_store) = PANE_DATA.lock() else {
        return;
    };
